    ...
    itemRow.innerHTML = `
        <div class="col-desc">
            <input
                type="text"
                class="item-desc"
                placeholder="Design &amp; art direction, week 1"
            >
        </div>
...
function calculateItemTotal(itemRow) {
    const qty =
        parseFloat(itemRow.querySelector('.item-qty').value) || 0;

    const price =
        parseFloat(itemRow.querySelector('.item-price').value) || 0;

    itemRow.querySelector('.item-amount').textContent =
        formatCurrency(qty * price);
}

function calculateTotals() {
    const itemRows =
        document.querySelectorAll('#itemsContainer .item-row');
...
function updateInvoicePreview() {
    ...
    setText(
        ...
        `${field('companyEmail') || 'contact@company.com'} · ${
            field('companyPhone') || '+1 (555) 123-4567'
        }`
    );
    ...
    setText(
        ...
        `${field('clientEmail') || 'contact@client.com'} · ${
            field('clientPhone') || '+1 (555) 987-6543'
        }`
    );
    ...
    setText(
        'previewInvoiceDate',
        invoiceDate ? formatDate(invoiceDate) : 'Jan 1, 2026'
    );
    ...
    setText(
        'previewDueDate',
        dueDate ? formatDate(dueDate) : 'Jan 15, 2026'
    );
    ...
    if (itemRows.length === 0) {
        previewItemsContainer.innerHTML =
            '<div class="preview-item-row empty-message">No items added yet</div>';
    } else {
        itemRows.forEach(row => {
            const desc =
            ...
            previewRow.innerHTML = `
                ...
                <span class="col-price mono-value">
                    ${formatCurrency(price)}
                </span>
                <span class="col-total mono-value">
                    ${amount}
                </span>
            `;
...
async function generatePdf() {
    ...
    btn.innerHTML =
        '<span class="stamp-ring">…</span> Generating…';
...
function clearAll() {
    if (!confirm('Clear every field on this invoice?')) {
        return;
    }
